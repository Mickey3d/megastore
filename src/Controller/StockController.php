<?php

namespace App\Controller;

use App\Entity\Stock;
use App\Form\Inventory\StockType;
use App\Repository\StockRepository;
use Symfony\Bundle\FrameworkBundle\Controller\Controller;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\Routing\Annotation\Route;

/**
 * @Route("/admin/stock")
 */
class StockController extends Controller
{
    /**
     * @Route("/", name="stock_index", methods="GET")
     */
    public function index(StockRepository $stockRepository): Response
    {
        return $this->render('Admin/Inventory/stock/index.html.twig', ['stocks' => $stockRepository->findAll()]);
    }

    /**
     * @Route("/new", name="stock_new", methods="GET|POST")
     */
    public function new(Request $request): Response
    {
        $stock = new Stock();
        $form = $this->createForm(StockType::class, $stock);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $em = $this->getDoctrine()->getManager();
            $em->persist($stock);
            $em->flush();
            $this->addFlash('success','The Stock have been created!');
            return $this->redirectToRoute('stock_index');
        }

        return $this->render('Admin/Inventory/stock/new.html.twig', [
            'stock' => $stock,
            'form' => $form->createView(),
        ]);
    }

    /**
     * @Route("/{id}", name="stock_show", methods="GET")
     */
    public function show(Stock $stock): Response
    {
        return $this->render('Admin/Inventory/stock/show.html.twig', ['stock' => $stock]);
    }

    /**
     * @Route("/{id}/edit", name="stock_edit", methods="GET|POST")
     */
    public function edit(Request $request, Stock $stock): Response
    {
        $form = $this->createForm(StockType::class, $stock);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $this->getDoctrine()->getManager()->flush();
            $this->addFlash('success','The Stock have been updated!');
            return $this->redirectToRoute('stock_edit', ['id' => $stock->getId()]);
        }

        return $this->render('Admin/Inventory/stock/edit.html.twig', [
            'stock' => $stock,
            'form' => $form->createView(),
        ]);
    }

    /**
     * @Route("/{id}", name="stock_delete", methods="DELETE")
     */
    public function delete(Request $request, Stock $stock): Response
    {
        if ($this->isCsrfTokenValid('delete'.$stock->getId(), $request->request->get('_token'))) {
            $em = $this->getDoctrine()->getManager();
            $em->remove($stock);
            $em->flush();
            $this->addFlash('success','The Stock have been deleted!');
        }

        return $this->redirectToRoute('inventory_index');
    }
}
